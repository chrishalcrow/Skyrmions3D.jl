...
function interactive_flow(my_skyrmion; iso_value = 2.0, kwargs...)
    ...
    g_plotting_options =
        g_info[1, 1] =
            GridLayout(tellwidth = false, tellheight = false, halign = :left, valign = :top)
    g_dynamics =
        g_info[2, 1] = GridLayout(
            tellwidth = false,
            tellheight = false,
            halign = :left,
            valign = :center,
    ...
    bd_tb = Textbox(
        g_plotting_options[2, 2];
        stored_string = string(iso_value),
        validator = Float64,
        tellwidth = false,
        halign = :left,
    )
    ...
    the_extrema = [extrema([BDmesh[a][1][b] for a = 1:size(BDmesh)[1]]) for b = 1:3]
    the_aspect = (
        the_extrema[1][2] - the_extrema[1][1],
        the_extrema[2][2] - the_extrema[2][1],
        the_extrema[3][2] - the_extrema[3][1],
    )

    ax = Axis3(g_skyrmion[1, 1], aspect = the_aspect, tellwidth = true; kwargs...)#, height=950, width=950 )


    Makie.mesh!(ax, obBD, color = obCM, shading = NoShading)
